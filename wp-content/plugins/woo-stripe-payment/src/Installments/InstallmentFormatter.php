...
class InstallmentFormatter {
	...
	public function format_plan( $plan, $amount, $currency ) {
		...
		$text   = sprintf( __( '%1$s / %2$s for %3$s', 'woo-paypal-payments' ), $this->format_price( $amount, [ 'currency' => $currency ] ), $this->get_formatted_interval( $plan->interval ), $this->get_formatted_duration( $plan->interval, $plan->count ) );
